...
# determines de occurrences for both classes
sorted_occurrences_3char = np.zeros((len(y_train_3char[0])))
for i in y_train_3char:
    sorted_occurrences_3char += np.array(i)

sorted_occurrences_full = np.zeros((len(y_train[0])))
for i in y_train:
    sorted_occurrences_full += np.array(i)
print('OCCURRENCES')
print(sorted_occurrences_full)
print(sorted_occurrences_3char)
...
clr = CyclicLR(base_lr=0.00001, max_lr=0.00006, step_size=2000.)


model.compile(loss=[categorical_class_balanced_focal_loss(n_instances_per_class=sorted_occurrences_full, beta=0.9, gamma=2.),
                    categorical_class_balanced_focal_loss(n_instances_per_class=sorted_occurrences_3char, beta=0.9, gamma=2.),
                    'binary_crossentropy'],
                    optimizer=Adam(0.0001), 
...
model.fit(X_train, [y_train, y_train_3char, y_train_aux], batch_size=batch_size, epochs=nb_epoch, 
          validation_data=(X_test, [y_test, y_test_3char, y_test_aux]), callbacks=[earlyStopping,clr,checkpoint])# TODO add metrics to callbacks
